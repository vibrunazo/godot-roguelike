...
func _ready() -> void:
	...
	if spikes_root == null:
		...
		get_tree().quit(1)
		return
	if spikes_root.position.y > -0.3:
		printerr("TEST FAILED: Spikes should be submerged initially (position.y <= -0.3), got: ", spikes_root.position.y)
		get_tree().quit(1)
		return
	print("SpikesRoot initial retracted position verified (Y = ", spikes_root.position.y, ")")
	
	# ---------------------------------------------------------
	# PART 2: Enemy Trigger Rejection
	# ---------------------------------------------------------
	print("\n>>> PART 2: Verifying Enemies Do NOT Trigger Spikes")
	...
	if not hazard.is_idle():
		printerr("TEST FAILED: Enemy triggered spikes hazard! Expected IDLE.")
		get_tree().quit(1)
		return
	print("Enemy walked over spikes without triggering (hazard remained IDLE).")
	
	# Move enemy slightly aside on the hazard edge
	enemy.global_position = Vector3(0.2, 1.0, 0.2)
	...
	# ---------------------------------------------------------
	# PART 3: Player Trigger & Dodge Delay Window
	# ---------------------------------------------------------
	print("\n>>> PART 3: Player Triggering & Dodge Window")
	...
	# During dodge window, no damage should be dealt yet
	...
	if hazard.damage_hitbox.monitoring:
		printerr("TEST FAILED: DamageHitbox became active before delay elapsed!")
		get_tree().quit(1)
		return
	print("Dodge window verified: No damage dealt during trigger_delay.")
	
	# ---------------------------------------------------------
	# PART 4: Spike Emergence & Damage to Player AND Enemy
	# ---------------------------------------------------------
	print("\n>>> PART 4: Spike Emergence & Multi-Target Damage")
	...
	if not damage_dealt:
		...
		get_tree().quit(1)
		return
	print("Player damage confirmed! HP: ", initial_player_hp, " -> ", player_health.current_health, " (took ", initial_player_hp - player_health.current_health, " damage)")
	
	if enemy_health.current_health >= initial_enemy_hp:
		printerr("TEST FAILED: Enemy in hitbox did not take damage from active spikes! Enemy HP: ", enemy_health.current_health)
		get_tree().quit(1)
		return
	print("Enemy damage confirmed! Enemy HP: ", initial_enemy_hp, " -> ", enemy_health.current_health, " (took ", initial_enemy_hp - enemy_health.current_health, " damage)")
	
	# ---------------------------------------------------------
	# PART 5: Retraction & Cooldown
	# ---------------------------------------------------------
	print("\n>>> PART 5: Spikes Retraction & Cooldown Cycle")
	# Move player and enemy away so hazard can complete cycle to IDLE
	player.global_position = Vector3(10.0, 1.0, 10.0)
	...
	print("  2. Enemies ignored by TriggerArea (no accidental trigger)")
	print("  3. Player trips TriggerArea on layer 32 (Triggers)")
	print("  4. Configurable trigger_delay provides dodge window")
	print("  5. Emerging spikes damage both Players and Enemies")
