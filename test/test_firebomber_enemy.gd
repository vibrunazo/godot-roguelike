...
func _ready() -> void:
	...
	if not (proj is EnemyProjectile):
		...
		get_tree().quit(1)
		return
	if proj.fall_gravity <= 0.0:
		printerr("TEST FAILED: Expected fall_gravity > 0.0, got: ", proj.fall_gravity)
		proj.queue_free()
		get_tree().quit(1)
		return
	if not is_equal_approx(proj.gravity, proj.fall_gravity * FirebombProjectile.EARTH_GRAVITY):
		printerr("TEST FAILED: Expected gravity == fall_gravity * EARTH_GRAVITY, got: ", proj.gravity)
		proj.queue_free()
		get_tree().quit(1)
		return
	if proj.speed <= 0.0:
		printerr("TEST FAILED: Expected speed > 0.0, got: ", proj.speed)
		proj.queue_free()
		get_tree().quit(1)
		return
	if proj.trap_duration <= 0.0:
		printerr("TEST FAILED: Expected trap_duration > 0.0, got: ", proj.trap_duration)
		proj.queue_free()
		get_tree().quit(1)
		return
	if proj.trap_size.x <= 0.0 or proj.trap_size.y <= 0.0:
		printerr("TEST FAILED: Expected positive trap_size, got: ", proj.trap_size)
		proj.queue_free()
		get_tree().quit(1)
		return
	if proj.fire_trap_scene == null:
		...
		get_tree().quit(1)
		return
	print("Export vars (speed, fall_gravity, trap_duration, trap_size, fire_trap_scene) verified.")
	proj.queue_free()
	await get_tree().process_frame
	...
	if col_box.size.x <= 0.0 or col_box.size.z <= 0.0:
		printerr("TEST FAILED: Collision box size must be positive, got: ", col_box.size)
		spawned_trap.queue_free()
		floor_body.queue_free()
		get_tree().quit(1)
		return
	print("Trap size (collision_shape=(", col_box.size, ")) verified.")

	if spawned_trap.duration <= 0.0:
		printerr("TEST FAILED: Expected positive duration, got: ", spawned_trap.duration)
		spawned_trap.queue_free()
		floor_body.queue_free()
		get_tree().quit(1)
		return
	print("Trap duration verified.")
	...
	if leap_dodge_state == null:
		...
		get_tree().quit(1)
		return
	if leap_dodge_state.cooldown <= 0.0:
		printerr("TEST FAILED: EnemyLeapingDodge.cooldown must be > 0.0, got: ", leap_dodge_state.cooldown)
		get_tree().quit(1)
		return
	if leap_dodge_state.max_range <= 0.0:
		printerr("TEST FAILED: EnemyLeapingDodge.max_range must be > 0.0, got: ", leap_dodge_state.max_range)
		get_tree().quit(1)
		return
	...
	if ai_dodge_state == null:
		...
		get_tree().quit(1)
		return
	if ai_dodge_state.cooldown <= 0.0:
		printerr("TEST FAILED: AILeapingDodge.cooldown must be > 0.0, got: ", ai_dodge_state.cooldown)
		get_tree().quit(1)
		return
	if ai_dodge_state.trigger_range <= 0.0:
		printerr("TEST FAILED: AILeapingDodge.trigger_range must be > 0.0, got: ", ai_dodge_state.trigger_range)
		get_tree().quit(1)
		return
	if ai_dodge_state.max_range <= 0.0:
		printerr("TEST FAILED: AILeapingDodge.max_range must be > 0.0, got: ", ai_dodge_state.max_range)
		get_tree().quit(1)
		return
	...
	print("EnemyLeapingDodge & AILeapingDodge node wiring and export configuration verified.")

	# ---------------------------------------------------------
	# PART 10: AI Trigger Conditions (Distance & Cooldown)
	# ---------------------------------------------------------
	print("\n>>> PART 10: AI Trigger Conditions (Distance & Cooldown)")
	...
	leap_player.position = leap_bomber.position + Vector3(0.0, 0.0, ai_dodge_state.trigger_range + 2.0)
	add_child(leap_player)
	leap_player.velocity = Vector3(0.0, -1.0, 0.0)
	leap_player.move_and_slide()

	# Case A: Player outside trigger_range (e.g. trigger_range + 2.0m away) - should NOT trigger
	leap_player.global_position = leap_bomber.global_position + Vector3(0.0, 0.0, ai_dodge_state.trigger_range + 2.0)
	...
	if far_trigger:
		printerr("TEST FAILED: AILeapingDodge triggered when player was outside trigger_range!")
		get_tree().quit(1)
		return
	print("Far player distance (> trigger_range) correctly rejected.")

	# Case B: Boundary test: Player at trigger_range + 0.2m - should NOT trigger
	leap_player.global_position = leap_bomber.global_position + Vector3(0.0, 0.0, ai_dodge_state.trigger_range + 0.2)
	await get_tree().physics_frame
	var boundary_far_trigger: bool = ai_dodge_state.evaluate_trigger(0.016)
	if boundary_far_trigger:
		printerr("TEST FAILED: AILeapingDodge triggered at boundary outside trigger_range!")
		get_tree().quit(1)
		return

	# Case C: Boundary test: Player within trigger_range - SHOULD trigger
	leap_player.global_position = leap_bomber.global_position + Vector3(0.0, 0.0, maxf(0.5, ai_dodge_state.trigger_range - 1.0))
	ai_dodge_state.cooldown_timer = 0.0
	var close_trigger: bool = ai_dodge_state.evaluate_trigger(0.016)
	if not close_trigger:
		printerr("TEST FAILED: AILeapingDodge did not trigger when player was within trigger_range!")
		get_tree().quit(1)
		return
	print("Near player distance (< trigger_range) successfully triggered AILeapingDodge.")
	...
	if ai_dodge_state.cooldown_timer < (ai_dodge_state.cooldown - 0.1) or ai_dodge_state.cooldown_timer > ai_dodge_state.cooldown:
		...
		get_tree().quit(1)
		return
	print("Cooldown timer initialized to configured cooldown.")
	...
	if not is_equal_approx(ai_dodge_state.cooldown_timer, prev_cd - 2.5):
		...
		get_tree().quit(1)
		return
	print("Cooldown timer decay verified.")
	...
	# Verify displacement is clamped within max_range
	var total_displacement: float = (leap_bomber.global_position - Vector3.ZERO).length()
	if total_displacement > leap_dodge_state.max_range + 0.1:
		printerr("TEST FAILED: Leaping dodge displaced ", total_displacement, "m, exceeding max_range of ", leap_dodge_state.max_range, "m!")
		get_tree().quit(1)
		return
	print("Leaping displacement safely within max range: ", total_displacement, "m.")
	...
	if clamped_dist > leap_dodge_state.max_range + 0.001:
		printerr("TEST FAILED: Explicit 50m target was not clamped to max_range. Got: ", clamped_dist)
		get_tree().quit(1)
		return
	print("Explicit target distance clamped to max_range: ", clamped_dist, "m.")
