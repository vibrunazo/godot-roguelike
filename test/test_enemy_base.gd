...
func _ready() -> void:
	...
	if level_enemy_health == null or not (level_enemy_health.max_health in [40.0, 60.0]):
		printerr("TEST FAILED: LevelTemplate Enemy HealthComponent missing or invalid max_health.")
		level.queue_free()
		get_tree().quit(1)
		return
	print("LevelTemplate Enemy HealthComponent confirmed with valid max health (", level_enemy_health.max_health, ").")
