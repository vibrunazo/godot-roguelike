...
func _ready() -> void:
	...
	var attack_damage: float = attack_state.damage
	
	# Wait for first hit
	for i: int in range(40):
		await get_tree().physics_frame
		if dummy_attrs.get_current(AttributeComponent.POOL_HEALTH) <= initial_health - attack_damage:
			break
			
	if not is_equal_approx(dummy_attrs.get_current(AttributeComponent.POOL_HEALTH), initial_health - attack_damage):
		printerr("TEST FAILED: First attack did not reduce health to expected value. Health: ", dummy_attrs.get_current(AttributeComponent.POOL_HEALTH))
		get_tree().quit(1)
		return
	print("Attack 1 hit confirmed! Dummy health: ", dummy_attrs.get_current(AttributeComponent.POOL_HEALTH))
	...
	for i: int in range(40):
		await get_tree().physics_frame
		if dummy_attrs.get_current(AttributeComponent.POOL_HEALTH) <= initial_health - (2.0 * attack_damage):
			break
			
	if not is_equal_approx(dummy_attrs.get_current(AttributeComponent.POOL_HEALTH), initial_health - (2.0 * attack_damage)):
		printerr("TEST FAILED: Second attack did not reduce health to expected value. Health: ", dummy_attrs.get_current(AttributeComponent.POOL_HEALTH))
		get_tree().quit(1)
		return
	print("Attack 2 hit confirmed! Dummy health: ", dummy_attrs.get_current(AttributeComponent.POOL_HEALTH))
	...
	print("  Final Dummy Health: ", dummy_attrs.get_current(AttributeComponent.POOL_HEALTH), " (Started at ", initial_health, ")      ")
	print("========================================================")
	level.queue_free()
