...
exit/hazard/litter placement and the VoxelGI volume. (Per-level pit quads
are obsolete: the template's giant abyss plane bottoms every hole, so
`extra_pits` is only honored for legacy specs.) The assembler copies the
template's structure (lighting, spawner, boundary, wiring), swaps in new
GridMap data + navmesh, inserts the new nodes, and recomputes every root
`index` so the override positions stay consistent.
...
def main() -> int:
    ...
    # Obsolete since the template abyss bottoms every hole: new specs omit
    # extra_pits entirely. The pattern lookup runs only when quads are
    # actually requested (old bases like level_3 no longer carry a Pit2).
    if spec.get("extra_pits"):
        pit_pattern = next((n for n in parse_nodes(base_text)
                            if n["name"] == spec.get("pit_pattern_from", "Pit2")),
                           None)
        assert pit_pattern is not None, (
            f"extra_pits needs pattern {spec.get('pit_pattern_from', 'Pit2')!r} "
            f"in {spec['template']}")
        _px, py, _pz = transform_origin(pit_pattern["text"])
        for pit in spec["extra_pits"]:
            blocks[pit["name"]] = clone_pattern(spec.get("pit_pattern_from", "Pit2"),
                                                pit["name"], float(pit["x"]), py,
                                                float(pit["z"]))
