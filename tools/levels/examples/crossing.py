#!/usr/bin/env python3
"""Worked example #6: a railless-bridge level (Level 9), "The Crossing".

Two rooms joined by a long OPEN bridge (rails="open": no rails at all) over
a 20 m void gap that no dash can clear, so the crossing is mandatory — and
knock-offs work from both deck edges. A fire trap sits mid-deck; the exit
room ends in an open east cliff. First shipped use of an open bridge.

Pipeline (mirrors the README):
    python tools/levels/examples/crossing.py --out-dir tools/levels/out/l9proof
...
# --- footprint: 6x6 rooms, 5-tile void gap, open 2x5 bridge (tiles) ---
# Edge letters are the generator's Godot-conventional sides (n=-Z north,
# s=+Z south, w=-X west, e=+X east). West room fully walled by derivation
# defaults; east room keeps an open east cliff for knock-off finishes.
ROOM_W = room(0, 5, 4, 9)
ROOM_E = room(11, 16, 4, 9, edge={"e": None})
SPAN = bridge(6, 10, 6, 7, rails="open")

# Freestanding cover pillars in the exit room (x, y, z, item, orient).
PILLARS = [
    (27, 0, 11, 0, 10), (27, 0, 16, 0, 10),
]
...
HAZARDS = [
    ("SpikesW", "spikes", 12, 22),
    ("FireDeck", "fire", 34, 30),
    ("SpikesE1", "spikes", 50, 22),
    ("SpikesE2", "spikes", 60, 34),
    ("FireE", "fire", 52, 32),
]

LITTER = [
    ("SpawnFlag", "flag", (18, 0, 20), 0),
    ("ExitFlag", "flag", (58, 0, 34), 0),
    ("BarrelW", "barrel", (6, 0, 34), 0),
    ("CouchW", "couch", (16, 0, 34), 90),
    ("BarrelE", "barrel", (64, 0, 20), 0),
    ("CouchE", "couch", (48, 0, 34), 0),
]

VOXELGI_POS = [34, 0, 28]
VOXELGI_SIZE = [76, 20, 32]


def main() -> int:
    ap = argparse.ArgumentParser(description="Design the Level 9 crossing.")
    ap.add_argument("--out-dir", required=True)
    ap.add_argument("--name", default="Level9")
    args = ap.parse_args()
    os.makedirs(args.out_dir, exist_ok=True)

    assert touches(SPAN.tiles, ROOM_W.tiles) and touches(SPAN.tiles, ROOM_E.tiles)
    floor, side_tiers = compose(ROOM_W, ROOM_E, SPAN)
    ...
    spec = {
        ...
        "navmesh_id": "NavigationMesh_level9",
        "strip_litter": True,
        "strip_hazards": True,
        "strip_pits": True,
        "exit": EXIT,
